...
public class CustomReporterListener extends CustomReportListenerAdapter{
    ...
    protected PrintWriter createWriter(String outdir) throws IOException {
        ...
        return new PrintWriter(new BufferedWriter(new FileWriter(new File(
                outdir, "qa-automation-report_SECURE"
//                + DateFunctions.dateToDayAndTimeForFileName(now)
    ...
    protected void generateMethodDetailReport(List<ISuite> suites) {
        ...
        for (ISuite suite : suites) {
            ...
            for (ISuiteResult r2 : r.values()) {
                ITestContext testContext = r2.getTestContext();
                if (r.values().size() > 0) {
                    m_out.println("<h2>" + testContext.getName() + "</h2>");
                }
    ...
    private void resultSummary(ISuite suite, IResultMap tests, String testname,
        ...
        if (tests.getAllResults().size() > 0) {
			...
			TestInfo annos=method.getMethod().getAnnotation(TestInfo.class);

             if (annos!= null) {
 	            try {
 	               // method1.invoke(login);
 	            	//System.out.println( annos3.tcName());
 	            	String tcnames= annos.tcName();
 	            	String features=annos.feature();
 	            	String expectedResults=annos.expectedResult();
 	               buff.append("<td>"+tcnames+"</td><td>"+features+"</td><td>"+expectedResults+"</td>");
 	               //annos.tcName()
            ...
            //buff.append("<td>"+tcnames+"</td><td></td><td>3</td>");
    ...
    private void startResultSummaryTable(String style) {
        ...
        m_out.println("<tr>"
        		+ "<th>Class</th>"
        		+ "<th>TC Name</th>"
                + "<th>Feature under<br>test</th>"
                + "<th>&nbsp;&nbsp;Expected Result&nbsp;&nbsp;</th>"
        		+ "<th>Method</th>"        		                
                //+ "<th># of<br/>Scenarios</th>"
                + "<th>Start</th>"
                + "<th>Time<br/>(ms)</th>"
                + "</tr>");
        m_row = 0;
    ...
    private void resultDetail(IResultMap tests) {
		...
		Set<ITestResult> ss= tests.getAllResults();
		List<ITestResult> results = Lists.newArrayList(ss);
     ...
     // for (ITestResult result : tests.getAllResults()) {
           ...
           ITestNGMethod method = result.getMethod();
            ...
            m_out.println("<h3 id=\"m" + m_methodIndex + "\">" + cname + ":"
                    + method.getMethodName() + "</h3>");
            Set<ITestResult> resultSet = tests.getResults(method);
...
//                 NEW
    ...
    protected void startHtml(PrintWriter out) {
        ...
        out.println("<head>");
        out.println("<title>Connectik.com - Automation Report</title>");
    ...
    protected void endHtml(PrintWriter out) {
        out.println("<center> Report customized by RootNext </center>");
